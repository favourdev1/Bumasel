...
const Sidebar = () => {
	...
	const userSidebarItems = [
		...
		},
		{
			id: 4,
			title: "Settings",
			icon: "", // Replace with actual icon component or class
			route: "/settings"
		},
		{
			id: 5,
	...
	return (
		<div className="bg-white border-r w-2/12  px-4 h-screen">
            ...
            >
                <RxDashboard
                    className={currentRoute.includes("/dashboard/vendor") ? "active" : ""}
                />
                <p className="text-sm font-semibold">Dashboard</p>
            </div>
            <div
                className={`flex items-center gap-4 py-2 ${
                    currentRoute.includes("/store") ? "bg-purple-700 text-white" : ""
                } hover:bg-purple-100 hover:ring-1 hover:ring-purple-200 duration-700 transition-all  hover:text-purple-700 rounded-lg px-4 cursor-pointer text-gray-700`}
                onClick={() => handleNavigation("/store")}
            >
                <ShopImageProp
                    className={`size-5 ${
                        currentRoute.includes("/store") ? "active" : ""
                    }`}
                />
            ...
            <div
                className={`flex items-center gap-4 py-2 ${
                    currentRoute.includes("/settings") ? "bg-purple-700 text-white" : ""
                } hover:bg-purple-100 hover:ring-1 hover:ring-purple-200 duration-700 transition-all  hover:text-purple-700 rounded-lg px-4 cursor-pointer text-gray-700`}
                onClick={() => handleNavigation("/settings")}
            >
                <div
                    className={`icon-placeholder ${
                        currentRoute.includes("/settings") ? "active" : ""
                    }`}
                ></div>
