      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        {products.map((item, index) => (
          <Link to={`/product/${index}`} key={index} className="w-full">
            <div
              key={index}
              className="bg-[#FAF7FF] transform transition-transform duration-300 hover:border-purple-700 hover:border-2 duration-500 overflow-hidden  border duration-1000 rounded-md"
            >
              <div className="relative overflow-hidden items-center justify-center flex w-full">
                <img
                  alt={item.title}
